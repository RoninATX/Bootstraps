#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# bootstrap-secure.sh — Ubuntu minimal bootstrap + fail2ban + unattended-upgrades
...
if [[ $EUID -ne 0 ]]; then
  echo "Please run as root (e.g., sudo -i; then bash bootstrap-secure.sh)"; exit 1
fi

# ---- Inputs ------------------------------------------------------------------
while true; do
  read -rp "New admin username: " NEWUSER
  [[ "$NEWUSER" =~ ^[a-z][-a-z0-9_]*$ ]] && ! id "$NEWUSER" &>/dev/null && break
  echo "Invalid or existing username. Use lowercase letters/digits/underscore, must start with a letter."
done

while true; do
  read -rsp "Password for ${NEWUSER}: " P1; echo
  read -rsp "Confirm password: " P2; echo
  [[ -n "$P1" && "$P1" == "$P2" ]] && break || echo "Passwords don't match. Try again."
done
